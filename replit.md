...
- **Timezone-Aware Availability (Nov 10):** Complete timezone fix using ISO strings throughout booking/rescheduling flow. `nextWeekdayFromSpeech()` replaces `resolveSpokenDayToRange()` and returns Brisbane-local ISO boundaries (`fromIso`, `toIso`, `day`). `getAvailability()` accepts ISO strings and converts to YYYY-MM-DD for Cliniko API. All routes (`book-part`, `book-choose`, `reschedule-part`) use consistent TZ constant and pass full payload objects. Verified with test: "wednesday" correctly parsed to next Wednesday with proper 24-hour ISO boundaries.
- **Multi-Appointment Reschedule & Cancel (Nov 10):** Replaced single-appointment reschedule flow with multi-appointment system. New flows: `res-list` → `res-pick-day` → `res-day` → `res-choose` for rescheduling; `cancel-list` → `cancel-confirm` for cancellations. Both flows display up to 2 upcoming appointments and let callers choose via DTMF (1/2) or speech. Uses `getPatientAppointments()` with robust phone lookup (`findPatientByPhoneRobust`) to support q= fallback for phone number variations. All flows maintain timezone consistency and include comprehensive error handling.
...
- **Enhanced Logging:** [AVAIL], [OFFER], [CHOOSE], [BOOK], [CONFIRM-YESNO], [RESCHEDULE] events for debugging booking flow.
